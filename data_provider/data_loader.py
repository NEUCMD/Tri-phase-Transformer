...
class Dataset_Wg(Dataset):
    ...
    def __read_data__(self):
        ...
        for gun_no in df_raw.size().index:
                ...
                train_dataset, train_labels = self.create_dataset(df_data, df_label, seq_len=self.seq_len, predict_time=self.pred_len, interval=600)
    ...
    # 构建特征数据集
    def create_dataset(self, x, y, class_num, seq_len=600, predict_time=600,  interval=600):
        features = []
        targets = []
        ...
        for i in range(0, len(x) - seq_len - predict_time, interval):
            ...
            multi_cls_lab = np.zeros(class_num, np.float32)
            ...
            if len(label_value) != 1:
                ...
                xx = y.iloc[i + seq_len - 400 : i + seq_len - 300]
                xxx_y = data.iloc[seq_len - 400 : seq_len - 300, 4].reset_index(drop=True)
                xxx_x = data.iloc[seq_len - 400: seq_len - 300, 4].reset_index(drop=True)
                ...
                plt.plot(plt_x, xxx_x)  # 绘制折线图，添加数据点，设置点的大小
                plt.plot(plt_x, xxx_y)  # 绘制折线图，添加数据点，设置点的大小
                plt.show()
...
class WQL(Dataset):
    def __init__(self, args, root_path, win_size, flag='train', size=None,
    ...
    def __read_data__(self):
        ...
        cols.remove('type')
        cols.remove('equipment_time')
        df_raw = df_raw[['equipment_time'] + cols]

        df_group = df_raw.groupby('equipment_id')
        ...
        for _, group in df_group:
            ...
            # 验证集数目是基于总数减去训练集和测试集的数目
            num_val = int(num_total - num_train - num_test)
            
            # 生成训练集、验证集和测试集
            train_set = group.iloc[:num_train]
            val_set = group.iloc[num_train:(num_train + num_val)]
            test_set = group.iloc[(num_train + num_val):]
            
            # 添加到相应的列表中
            train_frames.append(train_set)
            val_frames.append(val_set)
            test_frames.append(test_set)
        ...
        train_df = train_df.drop(columns=['equipment_time', 'equipment_id'])
        val_df = val_df.drop(columns=['equipment_time', 'equipment_id'])
        test_df = test_df.drop(columns=['equipment_time', 'equipment_id'])

        self.scaler.fit(train_df.values)
        train_df = self.scaler.transform(train_df.values)
        val_df = self.scaler.transform(val_df.values)
        test_df = self.scaler.transform(test_df.values)

        if self.set_type == 0:
            self.data_x = train_df
            self.data_y = train_df
        elif self.set_type == 1:
            self.data_x = val_df
            self.data_y = val_df
        else:
    ...
    def inverse_transform(self, data):
        # 假设 data 是通过PyTorch张量给出，并且具有形状 [批大小, 序列长度, 特征数]
        # 首先，把它转换为NumPy数组
        data_numpy = data.numpy()  # 如果数据在GPU上，使用data.cpu().numpy()

        # 准备一个空的数组用来保存逆变换后的数据
        inverted_data = np.empty_like(data_numpy)

        # 遍历批次中的每个样本
        for i in range(data_numpy.shape[0]):
            sample = data_numpy[i].reshape(-1, data_numpy.shape[-1])
            sample_inverted = self.scaler.inverse_transform(sample)
            inverted_data[i] = sample_inverted.reshape(data_numpy.shape[1], data_numpy.shape[2])

        # 如果需要，转换回PyTorch张量
        inverted_data_tensor = torch.tensor(inverted_data, dtype=torch.float)
        
        return inverted_data_tensor
